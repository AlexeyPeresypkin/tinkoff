...
class Payments(Base):
    ...
    language = mapped_column(
        Enum(Language, values_callable=lambda obj: [e.value for e in obj])
    )
    ...
    pay_type = mapped_column(
        Enum(PayType, values_callable=lambda obj: [e.value for e in obj])
    )
    ...
    cancelled_payments: Mapped[Optional["CancelledPayments"]] = relationship(
...
class CancelledPayments(Base):
    ...
    receipt: Mapped[Optional["Receipts"]] = relationship(
        back_populates="cancelled_payment"
    )
...
class Receipts(Base):
    ...
    taxation = mapped_column(
        Enum(Taxation, values_callable=lambda obj: [e.value for e in obj]),
        nullable=False,
    )
    additional_check_props: Mapped[Optional[str]]
    ffd_version = mapped_column(
        Enum(FfdVersion, values_callable=lambda obj: [e.value for e in obj])
    )
    payment_id: Mapped[Optional[int]] = mapped_column(ForeignKey("payments.id"))
    cancelled_payment_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("cancelled_payments.id")
    )
    payment: Mapped[Optional["Payments"]] = relationship(back_populates="receipt")
    cancelled_payment: Mapped[Optional["CancelledPayments"]] = relationship(
        back_populates="receipt"
    )
    items: Mapped[List["Items"]] = relationship(back_populates="receipt")
    receipt_payment: Mapped[Optional["ReceiptPayments"]] = relationship(
        back_populates="receipt"
    )
...
class Items(Base):
    ...
    payment_object = mapped_column(
        Enum(PaymentObject, values_callable=lambda obj: [e.value for e in obj])
    )
    tax = mapped_column(
        Enum(Tax, values_callable=lambda obj: [e.value for e in obj]), nullable=False
    )
...
class Agents(Base):
    ...
    __table_args__ = (
        UniqueConstraint("item_id", "id", name="agent_id_item_id_constr"),
    )

    id: Mapped[int] = mapped_column(primary_key=True)
    agent_sign = mapped_column(
        Enum(AgentSign, values_callable=lambda obj: [e.value for e in obj])
    )
...
class Suppliers(Base):
    ...
    __table_args__ = (
        UniqueConstraint("item_id", "id", name="supplier_id_item_id_constr"),
    )
