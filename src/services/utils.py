...
def get_token(data: dict) -> str:
    ...
    list_data = [
        ...
        if key not in exclude_keys and v
    ]
...
def to_camel(string: str) -> str:
    ...
    return camel_cases.get(string) or "".join(
        word.capitalize() if word not in ("url", "data", "ip") else word.upper()
        for word in string.split("_")
    )


def get_camel_case_data(data: dict) -> dict:
    """Конвертирует ключи payload в camel_case стиль для запроса"""
    for key in list(data):
        ...
        if isinstance(data_key, dict):
            ...
            data[to_camel(key)] = [
                get_camel_case_data(item) if isinstance(item, dict) else item
...
async def post_request(url: str, payload: json) -> dict:
